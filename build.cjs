...
const vuetifyExtension = {
  ...
  filter: function (text) {
    ...
    text = text.replace(/<import-code>([\S\s]*?)<\/import-code>/g, (match, importCode) => {
      ...
      let newText = `<v-sheet rounded="lg" style="padding: 1rem; margin-bottom: 1rem;"><div style="display: flex; justify-content: space-between; align-items: center;"><div style="font-family: Poppins; font-weight: 500;">Import Code</div><div style="display: flex; gap: .5rem;"><v-btn variant="tonal">Copy</v-btn><v-btn variant="tonal">Download</v-btn></div></div><br><div style="overflow-x: auto; background: #181818; padding: 1rem; border-radius: .5rem .5rem 0  0; color: #aac8e4;">${importCode}</div></v-sheet>`
