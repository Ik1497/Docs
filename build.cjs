...
const vuetifyExtension = {
  ...
  filter: function (text) {
    ...
    text = text.replace(/<import-code>([\S\s]*?)<\/import-code>/g, (match, importCode) => {
      ...
      let newText = `<v-sheet rounded="lg" style="padding: 1rem; margin-bottom: 1rem;"><br><div style="overflow-x: auto; background: #181818; padding: 1rem; border-radius: .5rem .5rem 0 0; color: #aac8e4;">${importCode}</div></v-sheet>`
